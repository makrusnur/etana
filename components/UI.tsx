...
export const Input: React.FC<React.InputHTMLAttributes<HTMLInputElement> & { label?: string }> = ({ label, className = '', ...props }) => (
  <div className="space-y-2 mb-4">
    ...
    <input className={cn("flex h-9 w-full rounded-md border border-slate-200 bg-transparent px-3 py-1 text-sm shadow-sm transition-colors placeholder:text-slate-500 focus-visible:outline-none focus-visible:ring-1 focus-visible:ring-slate-400", className)} {...props} />
  </div>
);
...
export const Select: React.FC<React.SelectHTMLAttributes<HTMLSelectElement> & { label?: string }> = ({ label, className = '', children, ...props }) => (
  <div className="space-y-2 mb-4">
    {label && <label className="text-sm font-medium leading-none">{label}</label>}
    <select className={cn("flex h-9 w-full items-center justify-between rounded-md border border-slate-200 bg-white px-3 py-2 text-sm shadow-sm focus:outline-none focus:ring-1 focus:ring-slate-400", className)} {...props}>{children}</select>
  </div>
);
  ...
  return (
    <div className="space-y-2">
      ...
      <div className="flex gap-2">
        <input type="number" placeholder="Tgl" className="w-14 h-11 rounded-xl border border-slate-200 text-center text-sm font-bold" value={d} onChange={e => handleChange(e.target.value, m, y)} />
        <select className="flex-1 h-11 rounded-xl border border-slate-200 px-3 text-sm font-bold bg-white" value={m} onChange={e => handleChange(d, e.target.value, y)}>
          <option value="">Bulan</option>
          {months.map(mo => <option key={mo.v} value={mo.v}>{mo.l}</option>)}
        </select>
        <input type="number" placeholder="Thn" className="w-20 h-11 rounded-xl border border-slate-200 text-center text-sm font-bold" value={y} onChange={e => handleChange(d, m, e.target.value)} />
      </div>
    </div>
  );
};